...
class ProfileData(object) :
    def __init__(self, name, op, start_time, dur, tid, tmam=None, children_list=[], input_name_list=[], core_list=[]) :
        self.__name = name
        self.__op = op
...
class TimelineData(object) :
            ...
            for elem in d :
                if 'cat' in elem :
                    if elem['cat'] == 'MTMC logs' :
                        self.__mtmc_prof.append(elem)

    def GetGroundtruthTmamD(self, name) :
        #print(name)
        return self.__groundtruth_tmam_d[name]

    # return the raw data of mtmc prof
    def GetMtmcProf(self) :
        return self.__mtmc_prof
    ...
    # process raw data of mtmc_prof and generate the parent/children mapping information
    def ProcessMtmcProf(self) :
        name_list = {}
        for elem in self.__mtmc_prof :
            if ('args' in elem) and ('step_id' in elem['args']) and ('node_name' in elem['args']):
                step_id = (elem['args']['step_id'])
                op_type = elem['name']
                tid = int(elem['tid'])
                start_time = float(elem['ts'])
                dur = float(elem['dur'])
                name = elem['args']['node_name']
                core_info = elem['args']['core_info']
                ...
                try :
                    ...
                    d = ProfileData(name, op_type, start_time, dur, tid,  t, list(), input_name_list, core_info)
    ...
    def GetCoreList(self, op_name) :
        core_list = []
        for elem in self.__mtmc_prof :
            if ('args' in elem) and ('core_info' in elem['args']) and ('node_name' in elem['args']) :
                if elem['args']['node_name'] == op_name :
                    core_list.extend(elem['args']['core_info'])
        return list(set(core_list))
...
class AutoAnalysis(object) :
    ...
    def _generate_suggestions(self, filter_info , output_file_name) :
        ...
        l2_core_info = {}
        l2_mem_node_names = []
        l2_fetch_latency_info = {}
        l2_fetch_latency_node_names = []
        for name in self._hotspot_name_list :
            tmam_d = t_tmam_d[name]
            l2_fetch_latency = round(np.percentile(tmam_d.GetLevel2FetchLatencyD().GetRawData(), 50), 2)
            l2_mem =  round(np.percentile(tmam_d.GetLevel2MemoryBoundD().GetRawData(), 50), 2)
            l2_core =  round(np.percentile(tmam_d.GetLevel2CoreBoundD().GetRawData(), 50), 2)
            if ((l2_fetch_latency > 20) and (name in filter_info)) :
                l2_fetch_latency_node_names.append(name)
                l2_fetch_latency_info[name] = l2_fetch_latency
            if ((l2_mem > 20) and (name in filter_info)) :
                l2_mem_node_names.append(name)
                l2_mem_info[name] = l2_mem
            l2_core_info[name] = l2_core

        suggestions = []
        for e in l2_mem_node_names :
            if e in l2_fetch_latency_node_names :
                core_info = t.GetCoreList(e)
                if len(core_info) > 2 and l2_core_info[e] < 10 :
                    c = "service core binding suggestion : " + e + " l2 core bound -- " + str(l2_core_info[e]) + "% , " \
                            + "l2 fetch latency bound -- " + str(l2_fetch_latency_info[e]) + "% , " \
                            + " l2 mem bound -- " + str(l2_mem_info[e]) + "% , running cores: " + str(core_info)
