...
class ThreadPoolTempl : public Eigen::ThreadPoolInterface {
 public:
  ...
  ThreadPoolTempl(int num_threads, bool allow_spinning,
...
#ifdef MTMC_PROFILER
                            this->tid_arr[i] = mtmc::MTMCTemprolProfiler::GetKtid();
                            cntr.fetch_add(1);
#endif
                            WorkerLoop(i); }));
    }
    ...
    std::vector<int64_t> GetPoolTids() {
#ifdef MTMC_PROFILER
//      std::vector<std::string> ret;
//      for (auto& tid : tid_arr) {
//          ret.push_back(std::to_string(tid));
//      }
      return tid_arr;
#else
      return {};
...
//    prof.Close();
  ...
  void ScheduleWithHint(std::function<void()> fn, int start,
                        int limit) override {
#ifdef MTMC_PROFILER
    auto ctx = *prof.GetCurrentCtx();
    auto parent_info = prof.GetParamsInfo(); // Parent
    parent_info.parent_ctx_hash_id = ctx.ctx_hash_id;
    auto fn_with_prof = [this, parent_info, fn
                         ,ctx
                         ] {
        std::string prefix = "~default~default~~~";
        if (!ctx.is_default) prefix = ctx.name;
        prof.UpdateCurrentCtx(ctx);

        prof.LogStart(parent_info, prefix); // Child
        fn();
        prof.LogEnd(); // Intra
